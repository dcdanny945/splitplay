...
export default function UpdateCardPage() {
  ...
  useEffect(() => {
    ...
    // Stripe sends people back here after the card is saved.
    if (params.get("done")) {
      setDone(true);
      setLoading(false);
      return;
    ...
    setToken(t);
    if (!t) {
      setInfo({ ok: false, error: "This link is missing its token." });
      setLoading(false);
      return;
    }
    fetch(`/api/update-card?token=${encodeURIComponent(t)}`)
      .then((r) => r.json())
      .then((d) => setInfo(d))
  ...
  const start = async () => {
    ...
    setBusy(false);
    setErr(d.error || "Could not start the card update");
  };
        ...
        <div style={{ background: "#fff", borderRadius: 20, padding: 32, border: "1px solid #e2e8f0", boxShadow: "0 4px 24px rgba(0,0,0,0.04)" }}>
          {loading ? (
            ...
            <div style={{ textAlign: "center" }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 800, marginTop: 8 }}>Your card is updated</div>
              <p style={{ fontSize: 14, color: "#64748b", marginTop: 8 }}>
                Your spot is unchanged — the new card will be used at settlement. The old one has been removed.
              </p>
            ...
            <div>
              <div style={{ fontSize: 20, fontWeight: 800, color: "#0f172a" }}>Change your card 💳</div>
              <div style={{ marginTop: 16, padding: 16, background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 14 }}>
                ...
                {info.card && (
                  <div style={{ fontSize: 13, color: "#64748b", marginTop: 2 }}>
                    Card on file: {info.card.brand.toUpperCase()} •••• {info.card.last4} (exp {String(info.card.expMonth).padStart(2, "0")}/{String(info.card.expYear).slice(-2)})
                  </div>
                )}
              </div>
              ...
              {info.canUpdate ? (
                <>
                  <p style={{ fontSize: 13, color: "#64748b", marginTop: 16 }}>
                    You&apos;ll go to Stripe to enter the new card. Nothing is charged now, and{" "}
                    <b>you keep your spot</b>{" "}— this doesn&apos;t remove you from the list.
                  </p>
                  <button
                    onClick={start}
                    disabled={busy}
                    style={{ width: "100%", marginTop: 8, padding: 14, borderRadius: 12, border: "none", background: busy ? "#94a3b8" : "linear-gradient(135deg, #06b6d4, #0d9488)", color: "#fff", fontSize: 15, fontWeight: 700, cursor: busy ? "default" : "pointer", fontFamily: "inherit" }}
                  >
                    {busy ? "Redirecting to Stripe…" : "Update my card"}
                  </button>
                ...
                <p style={{ fontSize: 13, color: "#991b1b", marginTop: 16, fontWeight: 600 }}>
                  {info.reason || "This card can no longer be changed."}
                </p>
              )}
            </div>
          )}
