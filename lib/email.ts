...
export type ConfirmationEmail = {
  ...
  mode: PaymentMode;
  splitCount?: number; // for split mode — how many the total was split among
};
...
export async function sendConfirmationEmail(opts: ConfirmationEmail): Promise<boolean> {
  ...
  <div style="font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;max-width:480px;margin:0 auto;padding:24px;color:#0f172a">
    ...
    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:18px">
      ...
      ${opts.splitCount ? `<div style="font-size:12px;color:#94a3b8;margin-top:8px;text-align:right">Total split evenly among ${opts.splitCount} participant${opts.splitCount === 1 ? "" : "s"}</div>` : ""}
    </div>
...
export async function sendRegistrationEmail(opts: RegistrationEmail): Promise<boolean> {
  ...
  <div style="font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;max-width:480px;margin:0 auto;padding:24px;color:#0f172a">
    <div style="font-size:20px;font-weight:800;color:#0d9488">Bball Court Fee</div>
    <h1 style="font-size:18px;margin:16px 0 4px">${opts.promoted ? "You're in — a spot opened up! ✅" : "You're registered ✅"}</h1>
    <p style="color:#475569;font-size:14px;margin:0 0 16px">Hi ${escapeHtml(opts.name)}, ${opts.promoted ? "a spot opened up and you've moved off the waitlist — you're confirmed!" : "your spot is saved."} Your card is stored securely on Stripe — you won't be charged until settlement.</p>
  ...
  try {
    await transporter.sendMail({
      from,
      to: opts.to,
      subject: `${opts.promoted ? "You're confirmed (moved up)" : "You're registered"} — ${opts.eventName}`,
      html,
    });
    return true;
  } catch (err) {
...
export async function sendWaitlistEmail(opts: WaitlistEmail): Promise<boolean> {
  if (!transporter) {
    console.warn(`[email] credentials not set — skipping waitlist email to ${opts.to}`);
    return false;
  }
  const from = process.env.EMAIL_FROM || `Bball Court Fee <${GMAIL_USER}>`;
  const html = `
  <div style="font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;max-width:480px;margin:0 auto;padding:24px;color:#0f172a">
    <div style="font-size:20px;font-weight:800;color:#0d9488">Bball Court Fee</div>
    <h1 style="font-size:18px;margin:16px 0 4px">You're on the waitlist ⏳</h1>
    <p style="color:#475569;font-size:14px;margin:0 0 20px">Hi ${escapeHtml(opts.name)}, this event is currently full, so you're on the waitlist. Your card is saved but <b>not charged</b>.</p>
    <div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:14px;padding:18px">
      <div style="font-size:12px;text-transform:uppercase;letter-spacing:1px;color:#94a3b8">Event</div>
      <div style="font-size:16px;font-weight:700;margin-top:2px">${escapeHtml(opts.eventName)}</div>
      ${opts.date ? `<div style="font-size:13px;color:#64748b;margin-top:8px">Date: ${escapeHtml(fmtDate(opts.date))}</div>` : ""}
    ...
    <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:14px;padding:16px;margin-top:16px">
      <p style="color:#1e40af;font-size:13px;margin:0;font-weight:600">If someone withdraws or a payment fails, you'll automatically move up — and we'll email you to confirm your spot.</p>
    </div>
    <div style="margin-top:20px;padding:16px;background:#fef2f2;border:1px solid #fecaca;border-radius:14px">
...
export async function sendFailedChargeEmail(opts: FailedChargeEmail): Promise<boolean> {
  if (!transporter) {
    console.warn(`[email] credentials not set — skipping failed-charge email to ${opts.to}`);
    return false;
  }
  const from = process.env.EMAIL_FROM || `Bball Court Fee <${GMAIL_USER}>`;
  const html = `
  <div style="font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;max-width:480px;margin:0 auto;padding:24px;color:#0f172a">
    ...
    <p style="color:#94a3b8;font-size:12px;margin-top:20px">If you think this was a mistake, please contact the organiser to sort it out.</p>
  </div>`;
  try {
    await transporter.sendMail({ from, to: opts.to, subject: `Payment couldn't be processed — ${opts.eventName}`, html });
    return true;
  } catch (err) {
